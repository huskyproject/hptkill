...
int makeRequestToLink(char * areatag, s_link * link)
{
    ...
    if(link->msg == NULL)
    {
        msg = makeMessage(link->ourAka,
                          ...
                          link->areafix.pwd ? link->areafix.pwd : "\x00",
                          1,
                          link->areafix.reportsAttr ? link->areafix.reportsAttr : robot->reportsAttr);
        msg->text = createKludges(config, NULL, link->ourAka, &(link->hisAka), "hptkill");
...
char * createDupeFileName(s_area * area)
{
    ...
    else
    {
        if(area->fileName)
        {
            xstrcat(&name, (ptr = strrchr(area->fileName, PATH_DELIM)) ? ptr + 1 : area->fileName);
        }
        else
        {
...
void update_queue(s_area * area)
{
    ...
    if(upDate == 0)
    {
        ...
        t2    = *localtime(&eTime);
        xscatprintf(&line,
                    "%s %s %d-%02d-%02d@%02d:%02d\t%d-%02d-%02d@%02d:%02d",
                    area->areaName,
                    "kill",
                    t1.tm_year + 1900,
                    t1.tm_mon + 1,
...
void delete_area(s_area * area)
{
    ...
    if(delFromConfig)
    {
        ...
        if(i < config->echoAreaCount && area == &(config->echoAreas[i]))
        {
            ...
            {
                memcpy(&(config->echoAreas[i]), &(config->echoAreas[i + 1]), sizeof(s_area));
            }
...
int main(int argc, char ** argv)
{
                ...
                case 'F':
                    ...
                    while(!feof(f))
                    {
                        ...
                        if(line)
                        {
                            ...
                            nareas++;
                            areas =
                                (char **)srealloc(areas, nareas * sizeof(char *));
                            areas[nareas - 1] = line;
                            needfree          =
                                (char *)srealloc(needfree, nareas * sizeof(char));
                            needfree[nareas - 1] = 1;
                        }
                    }
    ...
    if(MsgOpenApi(&m) != 0)
    {
        fprintf(outlog, "MsgApiOpen Error");
    }
    ...
    for(j = 0; j < nareas; j++)
    {
                ...
                else if((area->msgbType & MSGTYPE_PASSTHROUGH) == MSGTYPE_PASSTHROUGH)
                {
                    if(killNoLink)
                    {
                        ...
                        else if(checkPaused)
                        {
                            ...
                            {
                                if((area->downlinks[k]->link->Pause & ECHOAREA) != ECHOAREA)
                                {
                                    delArea--;
                                }
                            }
                            ...
                            else
                            {
                                ...
                                if(listNoDupeFile)
                                {
                                    if(!fNoDupe)
                                    {
                                        ...
                                        {
                                            fprintf(stderr,
                                                    "Unable to open file '%s' for appending\n",
                                                    listNoDupeFile);
                                        }
                                    }
        ...
        if(killPass == 0 && killLowLink == 0)
        {
            for(i = 0, area = config->localAreas; (unsigned int)i < config->localAreaCount;
                i++, area++)
            {
                if(patimat(area->areaName, areas[j]) == 1)
                {
            ...
            if(!found)
            {
                fprintf(outlog, "Unable to find area \"%s\"\n", areas[j]);
            }
        }
    }
